...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCryptbitAmount"))
        settings.setValue("nAnonymizeCryptbitAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeCryptbitAmount = settings.value("nAnonymizeCryptbitAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeCryptbitAmount"))
        SoftSetArg("-anonymizecryptbitamount", settings.value("nAnonymizeCryptbitAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeCryptbitAmount:
            nAnonymizeCryptbitAmount = value.toInt();
            settings.setValue("nAnonymizeCryptbitAmount", nAnonymizeCryptbitAmount);
            emit anonymizeCryptbitAmountChanged(nAnonymizeCryptbitAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
