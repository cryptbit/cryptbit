...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("cryptbit"))
        return false;
...
{
    // Convert cryptbit:// to cryptbit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cryptbit://"))
    {
        uri.replace(0, 12, "cryptbit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Cryptbit.lnk";
}
...
{
    return GetAutostartDir() / "cryptbit.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Cryptbit-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  cryptbit-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(96,96,96); color: rgb(255,255,255); border-radius: 3px; padding-left: 5px; padding-right: 5px; height: 21px; width: 100%; text-align: center; font-size: 11px; }"
                      ...
                      "QToolButton    { background: rgb(96,96,96); font-size: 14px; color: black; border: none; margin-top: 8px; margin-bottom: 8px; }"
                      ...
                      "QToolBar       { background: rgb(96,96,96); border: none; max-width: 160px; }");
}
