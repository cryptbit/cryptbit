...
//
// Bootup the masternode, look for a 13342 CBIT input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        LogPrintf("CActiveMasternode::ManageStatus() - Checking inbound connection to '%s'\n", service.ToString().c_str());
          ...
          // CBITNOTE: There is no logical reason to restrict this to a specific port.  Its a peer, what difference does it make.
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    {
        if(out.tx->vout[out.i].nValue == 13342*COIN) { //exactly
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == 13342*COIN) { //exactly
        	filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
