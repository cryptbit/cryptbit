    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Cryptbit and make it unusable. To
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeCryptbitAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fCryptbitSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f DRK CENT - sessionTotalValue %d\n", fCryptbitSubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("cryptbit-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeCryptbitAmount = (rand() % nLeftToAnon)+3;
                } else {
